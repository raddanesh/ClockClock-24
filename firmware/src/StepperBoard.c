...
typedef struct {
  uint8_t addr;
  bool enabled;
  STEPPER_Handle_t stepper[STEPBOARD_NOF_CLOCKS][STEPPER_NOF_CLOCK_MOTORS];
} STEPBOARD_Device_t;
...
static const STEPBOARD_Config_t defaultConfig =
{
  ...
  .stepper[3][0] = NULL, .stepper[3][1] = NULL,
	.stepper[4][0] = NULL, .stepper[4][1] = NULL,
	.stepper[5][0] = NULL, .stepper[5][1] = NULL,
};
...
STEPBOARD_Handle_t STEPBOARD_InitDevice(STEPBOARD_Config_t *config) {
  ...
  if (handle!=NULL) { /* if malloc failed, will return NULL pointer */
    ...
    handle->stepper[4][0] = config->stepper[4][0];
    handle->stepper[4][1] = config->stepper[4][1];
    handle->stepper[5][0] = config->stepper[5][0];
    handle->stepper[5][1] = config->stepper[5][1];
  }
  return handle;
}
...
bool STEPBOARD_ItemsInQueue(STEPBOARD_Handle_t board) {
  ...
  for(int i=0; i<STEPBOARD_NOF_CLOCKS; i++) {
