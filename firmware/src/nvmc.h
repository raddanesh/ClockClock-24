...
typedef struct {
  ...
  int16_t zeroOffsets[6][2]; /* two offsets for each motor, offset from the magnet sensor to the zero position */
  int16_t backlashsteps[6][2]; /* backlash steps for inner and outer shafts */
  /* fill up to 64 bytes, needed for flash programming */
  uint8_t filler[64-4-1-(6*2*2)-(6*2*2)-4];
} NVMC_Data_t;
