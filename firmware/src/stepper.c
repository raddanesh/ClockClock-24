...
#define STEPPER_START_TIMER()        SCTIMER_StartTimer(SCT0, kSCTIMER_Counter_U)
#define STEPPER_STOP_TIMER()         SCTIMER_StopTimer(SCT0, kSCTIMER_Counter_U)
...
typedef struct {
  ...
  int32_t (*getBacklashFn)(void *device, Coord_t coord, int steps); /* function pointer to get backlash steps */
  int32_t lastBacklashMove;
  int32_t degree; /* current degree */
...
STEPPER_Handle_t STEPPER_InitDevice(STEPPER_Config_t *config) {
  ...
  if (handle!=NULL) { /* if malloc failed, will return NULL pointer */
    ...
    handle->getBacklashFn = config->getBacklashFn;
    handle->degree = 0;
    handle->pos = 0;
    handle->doSteps = 0;
    handle->delay = 0;
...
static void Timer_Init(void) {
  ...
  SCTIMER_SetupCounterLimitAction(SCT0, kSCTIMER_Counter_U, eventNumberOutput);
...
void STEPPER_MoveClockDegreeAbs(STEPPER_Handle_t stepper, int32_t degree, STEPPER_MoveMode_e mode, uint8_t delay, bool speedUp, bool slowDown) {
  ...
  if (device->degree == 0 && currPos != 0) {
  //if (device->degree == 361) {
  	currDegree = ((currPos - device->lastBacklashMove) *360)/STEPPER_CLOCK_360_STEPS; /* current angle: 0..359 */
  } else {
  	currDegree = device->degree;
  }
  ...
  int32_t backlashSteps = device->getBacklashFn(device->device, device->coord, steps);

  device->degree = targetDegree;
  device->lastBacklashMove = backlashSteps;
  device->doSteps = steps + backlashSteps;
...
Coord_t STEPPER_GetCoord(STEPPER_Handle_t stepper) {
	STEPPER_Device_t *device = (STEPPER_Device_t*)stepper;
	return device->coord;
}
...
void STEPPER_StrCatStatus(STEPPER_Handle_t stepper, unsigned char *buf, size_t bufSize) {
  ...
  normPos = STEPPER_NormalizePos(stepper);
  ...
  McuUtility_strcat(buf, bufSize, (unsigned char*)", norm:");
  McuUtility_strcatNum16sFormatted(buf, bufSize, normPos, ' ', 2);
