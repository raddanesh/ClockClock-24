...
//#define FLASH_NVM_ADDR_START      (0xFE00) /* LPC845 has 64k FLASH (0x10000), last 1k page is used for NVM */
...
static uint8_t NVMC_InitConfig(void) {
  ...
  for (int i=0; i<sizeof(data.backlashsteps)/sizeof(data.backlashsteps[0]); i++) {
    for(int j=0; j<sizeof(data.backlashsteps[0])/sizeof(data.backlashsteps[0][0]); j++) {
      data.backlashsteps[i][j] = 0; /* default backlash steps */
    }
  }
...
int16_t NVMC_GetStepperBacklashSteps(uint8_t clock, uint8_t motor) {
  const NVMC_Data_t *data;

  data = NVMC_GetDataPtr();
  if (data!=NULL) {
    return data->backlashsteps[clock][motor];
  }
  return 0; /* default */
}
...
static uint8_t PrintStatus(const McuShell_StdIOType *io) {
  ...
  if (NVMC_IsErased()) {
    ...
    McuUtility_strcpy(buf, sizeof(buf), (unsigned char*)"In   Out\r\n");
    ...
    for (int i=0; i<sizeof(data->backlashsteps)/sizeof(data->backlashsteps[0]); i++) {
      buf[0] = '\0';
      for(int j=0; j<sizeof(data->backlashsteps[0])/sizeof(data->backlashsteps[0][0]); j++) {
        McuUtility_strcatNum16sFormatted(buf, sizeof(buf), data->backlashsteps[i][j], ' ', 5);
        McuUtility_strcat(buf, sizeof(buf), (unsigned char*)" ");
      }
      McuUtility_strcat(buf, sizeof(buf), (unsigned char*)"\r\n");
      McuUtility_strcpy(status, sizeof(status), (unsigned char*)"  bcklsh[");
      McuUtility_strcatNum16s(status, sizeof(status), i);
      McuUtility_strcat(status, sizeof(status), (unsigned char*)"]");
      McuShell_SendStatusStr(status, buf, io->stdOut);
    }
